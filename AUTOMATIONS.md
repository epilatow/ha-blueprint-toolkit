...
- **Translations.** Each repair spec sets `translation_key=<kind>`; the
  ...
  name-drift names) and `` `<entity_id>` (<name>) `` for DW disabled
  diagnostics (the DW notification body and repair use identical entity-line
  text). Single-entity findings pass the fields they need instead -- EDW's
  script-yaml-key repair passes `{entity_id}` / `{old_key}` / `{new_slug}`.
